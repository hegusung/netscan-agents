...
fnGetNativeSystemInfo pGetNativeSystemInfo;
fnHeapAlloc pHeapAlloc;
fnGetProcessHeap pGetProcessHeap;
fnHeapFree pHeapFree;
fnGetThreadLocale pGetThreadLocale;
fnlstrlenA plstrlenA;
fnIsBadReadPtr pIsBadReadPtr;
...
BOOL init_memorymodule()
{
    ...
    pLoadLibraryA = (fnVirtualFree)GetProcAddress_byhash(kernel32, LoadLibraryA_hash);
    pGetProcAddress = (fnVirtualFree)GetProcAddress_byhash(kernel32, GetProcAddress_hash);
    pFreeLibrary = (fnVirtualFree)GetProcAddress_byhash(kernel32, FreeLibrary_hash);
    ...
    pHeapFree = (fnHeapFree)GetProcAddress_byhash(kernel32, HeapAlloc_hash);
...
FinalizeSection(PMEMORYMODULE module, PSECTIONFINALIZEDATA sectionData) {
    ...
    if (pVirtualProtect(sectionData->address, sectionData->size, protect, &oldProtect) == 0) {
...
{
    UNREFERENCED_PARAMETER(userdata);
    return (FARPROC) pGetProcAddress((HMODULE) module, name);
}

void MemoryDefaultFreeLibrary(HCUSTOMMODULE module, void *userdata)
{
    UNREFERENCED_PARAMETER(userdata);
    pFreeLibrary((HMODULE) module);
}
    ...
    void *userdata)
{
    ...
    result = (PMEMORYMODULE)pHeapAlloc(pGetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(MEMORYMODULE));
    if (result == NULL) {
        freeMemory(code, 0, MEM_RELEASE, userdata);
#ifdef _WIN64
        FreePointerList(blockedMemory, freeMemory, userdata);
    ...
    LPCTSTR key)
{
    ...
    if (!IS_INTRESOURCE(key) && key[0] == TEXT('#')) {
        ...
        if (tmpkey <= 0xffff && plstrlenA(endpos) == 0) {
...
HMEMORYRSRC MemoryFindResourceEx(HMEMORYMODULE module, LPCTSTR name, LPCTSTR type, WORD language)
{
    ...
    if (language == DEFAULT_LANGUAGE) {
        ...
        language = LANGIDFROMLCID(pGetThreadLocale());
    }
